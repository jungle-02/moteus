...
private:
  ...
  void ISR_DoCurrent(const SinCos &sin_cos, float i_d_A_in, float i_q_A_in,
            ...
            MOTEUS_CCM_ATTRIBUTE {
              ...
              if (!std::isnan(position_config_.position_max) &&
                ...
                return std::make_pair(
                    in * std::max(0.0f, 1.0f - (position_.position -
    ...
    const float we = position_.velocity / motor_position_->config()->rotor_to_output_ratio * motor_.poles * 0.5f * k2Pi;
  ...
  void ISR_DoPositionCommon(const SinCos &sin_cos, CommandData *data,
    ...
    const float measured_velocity =
        velocity_command + Threshold(position_.velocity - velocity_command,
    ...
    const float unlimited_torque_Nm =
        (pid_position_.Apply(
             (static_cast<int32_t>((position_.position_relative_raw -
                                    *status_.control_position_raw) >>
                                   32) /
              65536.0f),
