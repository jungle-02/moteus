...
#include "fw_smo2/bldc_servo_position.h"
#include "fw_smo2/foc.h"
#include "fw_smo2/moteus_math.h"
#include "fw_smo2/moteus_hw.h"
#include "fw_smo2/sensorless_observer.h"
#include "fw_smo2/stm32g4_adc.h"
#include "fw_smo2/thermistor.h"
#include "fw_smo2/torque_model.h"
#include "fw_smo2/parameter_estimator.h"

#if defined(TARGET_STM32G4)
#include "fw_smo2/stm32g4_async_uart.h"
#else
...
std::pair<float, errc> LimitCode(float, float, float, errc,
                                 errc) MOTEUS_CCM_ATTRIBUTE;

std::pair<float, errc> LimitCode(float a, float min, float max, errc code_limit,
                                 errc code_nolimit) {
  if (a < min) {
    return {min, code_limit};
  }
  if (a > max) {
    return {max, code_limit};
  }
...
template <typename Array> int MapConfig(const Array &array, int value) {
...
struct RateConfig {
  ...
  RateConfig(int pwm_rate_hz_in = 30000) {
    const int board_min_pwm_rate_hz =
        (g_measured_hw_family == 0 && g_measured_hw_rev == 2) ? 60000 : 15000;

    // Limit our PWM rate to even frequencies between 15kHz and 60kHz.
    pwm_rate_hz =
        ((std::max(board_min_pwm_rate_hz, std::min(60000, pwm_rate_hz_in))) /
         2) *
        2;
...
constexpr float kSensorlessSwitchHz = 1.5f; // Nguong toc do (Hz co hoc) de bat dau dung Sensorless (SMO)
...
public:
  ...
  bool read() { return (*reg_in_ & mask_) != 0; }
...
public:
  ...
  ExponentialFilter(float rate_hz, float period_s)
      : alpha_(1.0f / (rate_hz * period_s)), one_minus_alpha_(1.0f - alpha_) {}
...
public:
  Impl(micro::PersistentConfig *persistent_config,
       ...
       MillisecondTimer *millisecond_timer, MotorDriver *motor_driver,
       AuxADC *aux_adc, AuxPort *aux1_port, AuxPort *aux2_port,
       MotorPosition *motor_position, const Options &options)
      : options_(options), ms_timer_(millisecond_timer),
        motor_driver_(motor_driver), aux_adc_(aux_adc), aux1_port_(aux1_port),
        aux2_port_(aux2_port), motor_position_(motor_position),
        pwm1_(options.pwm1), pwm2_(options.pwm2), pwm3_(options.pwm3),
        phase_monitors_(options.pwm1, options.pwm2, options.pwm3),
        current1_(options.current1), current2_(options.current2),
        current3_(options.current3), vsense_(options.vsense),
        vsense_sqr_(FindSqr(options.vsense)), tsense_(options.tsense),
        tsense_sqr_(FindSqr(options.tsense)), msense_(options.msense),
        msense_sqr_(FindSqr(options.msense)), debug_dac_(options.debug_dac),
  ...
  void Start() {
    ...
    if (options_.debug_uart_out != NC) {
      const auto uart =
          pinmap_peripheral(options_.debug_uart_out, PinMap_UART_TX);
      debug_uart_ = onboard_debug_uart_ =
          reinterpret_cast<USART_TypeDef *>(uart);
    }
  }

  ~Impl() { g_impl_ = nullptr; }

  void Command(const CommandData &data) {
    if (data.mode == kFault || data.mode == kEnabling ||
        data.mode == kCalibrating || data.mode == kCalibrationComplete) {
    ...
    *next = data;
    ...
    if (!std::isnan(next->velocity_limit) && !std::isnan(next->velocity)) {
      next->velocity =
          Limit(next->velocity, -next->velocity_limit, next->velocity_limit);
    }

    // Transform any position and stop_position command into the
    // relative raw space.
    const auto delta =
        static_cast<int64_t>(motor_position_->absolute_relative_delta.load())
        << 32ll;
    ...
    if (!next->position_relative_raw && !!next->stop_position_relative_raw &&
        !std::isnan(next->velocity) && next->velocity != 0.0f) {

      next->velocity =
          std::abs(next->velocity) * (((*next->stop_position_relative_raw -
    ...
    if (config_.bemf_feedforward != 0.0f && !std::isfinite(next->accel_limit) &&
    ...
    next->synthetic_theta = config_.fixed_voltage_mode ||
  ...
  bool is_torque_constant_configured() const { return motor_.Kv != 0.0f; }

  float current_to_torque(float current) const MOTEUS_CCM_ATTRIBUTE {
    TorqueModel model(torque_constant_, motor_.rotation_current_cutoff_A,
...
//    velocity_filter_ = ExponentialFilter(rate_config_.pwm_rate_hz, 0.01f);
	// --- SỬA ĐỔI: LÀM MƯỢT BỘ LỌC VẬN TỐC --- (SMO)
	// Tăng thời gian lọc (giảm tần số cắt) để làm mượt tín hiệu vận tốc từ SMO
	// Giá trị 0.05f (50ms) có thể làm giảm độ nhạy nhưng sẽ giúp triệt tiêu gai (chatter)
    velocity_filter_ = ExponentialFilter(rate_config_.pwm_rate_hz, 0.01f);
    ...
    const float max_sensable_current_A = kSensableCurrentMargin *
                                         motor_driver_->max_sense_V() /
                                         config_.current_sense_ohm;
    config_.max_current_A =
        std::min(config_.max_current_A, max_sensable_current_A);
    ...
    torque_constant_ = is_torque_constant_configured()
                           ? kTorqueFactor / motor_.Kv
                           : kDefaultTorqueConstant;
    v_per_hz_ = motor_.Kv == 0.0f ? 0.0f : 0.5f * 60.0f / motor_.Kv;

    adc_scale_ =
        3.3f / (4096.0f * config_.current_sense_ohm * motor_driver_->i_gain());
    ...
    // Cap nhat dt cho SMO
    sensorless_observer_.SetRate(rate_config_.period_s);
  ...
  void PollMillisecond() {
    ...
    if (motor_position_->status().epoch != main_motor_position_epoch_) {
...
private:
  void ConfigurePwmIrq() {
    ...
    NVIC_SetVector(PendSV_IRQn,
                   reinterpret_cast<uint32_t>(&Impl::GlobalPendSv));
  ...
  void ConfigurePwmTimer() {
    ...
    MJ_ASSERT(pwm1_timer != 0 && pwm1_timer == pwm2_timer &&
  ...
  void ConfigureADC() {
    ...
    auto make_cycles = [](auto v) {
      return (v << 0) | (v << 3) | (v << 6) | (v << 9) | (v << 12) | (v << 15) |
             (v << 18) | (v << 21) | (v << 24);
    };
    ...
    if (family0_) {
      adc1_sqr_ = ADC1->SQR1 = (0 << ADC_SQR1_L_Pos) | // length 1
                               FindSqr(options_.current2) << ADC_SQR1_SQ1_Pos;
      adc2_sqr_ = ADC2->SQR1 = (0 << ADC_SQR1_L_Pos) | // length 1
                               FindSqr(options_.current3) << ADC_SQR1_SQ1_Pos;
      adc3_sqr_ = ADC3->SQR1 = (0 << ADC_SQR1_L_Pos) | // length 1
    ...
    if (family0_rev4_and_older_) {
      ...
      // battery sense first.
      adc4_sqr_ = ADC4->SQR1 = (0 << ADC_SQR1_L_Pos) | // length 1
                               (msense_sqr_ << ADC_SQR1_SQ1_Pos);
      ADC5->SQR1 = (0 << ADC_SQR1_L_Pos) | // length 1
  ...
  void ISR_HandleTimer() __attribute__((always_inline)) MOTEUS_CCM_ATTRIBUTE {
    ...
    if ((sr & TIM_SR_UIF) && (cr & TIM_CR1_DIR)) {
      ISR_DoTimer();
    }
  }
  ...
  void ISR_DoTimerLowerPriority()
      __attribute__((always_inline)) MOTEUS_CCM_ATTRIBUTE {
    ...
    CommandData *data = current_data_;
        ...
        // --- HYBRID CONTROL: ENCODER -> SENSORLESS --- (SMO)
        float electrical_theta;
        if (data->synthetic_theta) {
          electrical_theta = WrapZeroToTwoPi(
            ...
            const float fading_band = 0.5f;

            // 1. CHUYỂN TRẠNG THÁI MƯỢT MÀ
            float enc_mech_hz_abs = std::abs(status_.velocity_filt / motor_position_->config()->rotor_to_output_ratio);
            float smo_mech_hz_abs = std::abs(status_.sensorless_estimated_velocity);
            float ref_vel_hz = (sensorless_alpha_blend_ >= 1.0f) ? smo_mech_hz_abs : enc_mech_hz_abs;

            float alpha = 0.0f;
            if (ref_vel_hz >= kSensorlessSwitchHz) {
              alpha = 1.0f;
            } else if (ref_vel_hz > (kSensorlessSwitchHz - fading_band)) {
              alpha = (ref_vel_hz - (kSensorlessSwitchHz - fading_band)) / fading_band;
            }
            sensorless_alpha_blend_ = alpha;

            // 2. BLEND GÓC ĐIỆN TỪ (Cho Vòng lặp Dòng điện) - Chạy bằng 100% SMO ở tốc độ cao
            float smo_theta_elec = WrapZeroToTwoPi(sensorless_observer_.state().estimated_theta * (motor_.poles * 0.5f) * k2Pi);
            float enc_theta_elec = position_.electrical_theta;

            if (alpha >= 1.0f) {
                electrical_theta = smo_theta_elec;
            } else if (alpha <= 0.0f) {
                electrical_theta = enc_theta_elec;
            } else {
                float diff = smo_theta_elec - enc_theta_elec;
                if (diff > kPi) diff -= k2Pi;
                if (diff < -kPi) diff += k2Pi;
                electrical_theta = WrapZeroToTwoPi(enc_theta_elec + alpha * diff);
            }

            // 3. BLEND VẬN TỐC (Cho Vòng lặp Vận tốc & Khâu Đạo hàm PID) - Dùng bộ tích phân siêu mượt của SMO
            float enc_smooth_out_hz = status_.velocity_filt / motor_position_->config()->rotor_to_output_ratio;
            float smo_smooth_rotor_hz = (sensorless_observer_.state().we_int / (motor_.poles * 0.5f)) / (2.0f * kPi);
            float smo_smooth_out_hz = smo_smooth_rotor_hz * motor_position_->config()->rotor_to_output_ratio * motor_position_config()->output.sign;

            float active_smooth_out_hz = enc_smooth_out_hz * (1.0f - alpha) + smo_smooth_out_hz * alpha;
            active_mech_hz_ = active_smooth_out_hz / motor_position_->config()->rotor_to_output_ratio;

            status_.velocity = active_smooth_out_hz;

            // 4. KIỂM SOÁT VỊ TRÍ TUYỆT ĐỐI (Cho Vòng lặp Vị trí)
            // ĐÂY LÀ CHÌA KHÓA: Vòng ngoài cùng luôn dùng Encoder để triệt tiêu hoàn toàn sự trôi dạt và rung lắc.
            // Điều này vẫn đảm bảo động cơ được lái bằng sức mạnh của SMO ở vòng trong, nhưng cập bến chính xác tuyệt đối.
            status_.position = position_.position;
            active_position_relative_raw_ = position_.position_relative_raw;
        }
    ...
    status_.final_timer = ((*timer_cr1_) & TIM_CR1_DIR) ? (pwm_counts_ - cnt)
                                                        : (pwm_counts_ + cnt);
  ...
  void ISR_DoSenseCritical()
    ...
    if (status_.mode != kFault && phase_monitors_.read()) {
      status_.mode = kFault;
  ...
  void ISR_DoSense() __attribute__((always_inline)) MOTEUS_CCM_ATTRIBUTE {
...

    {
      ...
      if (config_.enable_motor_temperature) {
        status_.motor_temp_C =
            motor_thermistor_.Calculate(status_.adc_motor_temp_raw);
  ...
  void ISR_CalculateCurrentState(const SinCos &sin_cos) MOTEUS_CCM_ATTRIBUTE {
    status_.cur1_A =
        (status_.adc_cur1_raw - status_.adc_cur1_offset) * adc_scale_;
    status_.cur2_A =
        (status_.adc_cur2_raw - status_.adc_cur2_offset) * adc_scale_;
    status_.cur3_A =
        (status_.adc_cur3_raw - status_.adc_cur3_offset) * adc_scale_;
    ...
    DqTransform dq{sin_cos, status_.cur1_A, status_.cur3_A, status_.cur2_A};
    status_.d_A = dq.d;
    ...
    status_.motor_max_velocity =
        motor_position_->config()->rotor_to_output_ratio *
        rate_config_.max_voltage_ratio * kVelocityMargin * 0.5f *
        status_.filt_1ms_bus_V / v_per_hz_;
    ...
    status_.max_power_W = [&]() {
      ...
      const float board_power_limit =
          Limit(Interpolate(status_.filt_bus_V, g_hw_pins.power_V_l,
                            g_hw_pins.power_V_h, g_hw_pins.power_P_l_W,
                            g_hw_pins.power_P_h_W),
                g_hw_pins.power_P_h_W, g_hw_pins.power_P_l_W) *
          pwm_derate_;
    ...
    DAC1->DHR12R1 = static_cast<uint32_t>(dq.d * 400.0f + 2048.0f);
		...
		// ===== CAP NHAT SMO =====
		// Observer duoc goi o day voi:
		// - Dong dien d/q hien tai (status_.d_A, status_.q_A)
		// - Dien ap d/q tu chu ky truoc (old_d_V, old_q_V)
		// - Tham so motor (resistance, Kv)
		// - Encoder angle de khoi tao
				sensorless_observer_.Update(
                    status_.d_A,
					dq.q,
                    old_d_V,
					old_q_V * motor_position_config()->output.sign,
                    rls_R_hat_,                         // R tu bo uoc luong hoac thong so tinh
                    rls_L_hat_,                         // L tu bo uoc luong hoac thong so tinh
                    motor_.poles,
					old_sin,
                    old_cos,
                    is_torque_on                        // Co kich hoat observer
                );
				/// Tính Vận Tốc và Góc VẬT LÝ để Align bằng biến đã lọc nhiễu
				const float phys_elec_vel_rads = motor_position_config()->output.sign * (position_.velocity / motor_position_->config()->rotor_to_output_ratio) * motor_.poles * 0.5f * k2Pi;
				const float phys_rotor_mech_revs = motor_position_config()->output.sign * (position_.position / motor_position_->config()->rotor_to_output_ratio);

				// --- KHÔI PHỤC ĐỒNG BỘ Ở TỐC ĐỘ THẤP ---
				if (std::abs(status_.velocity_filt) < kSensorlessSwitchHz) {
					sensorless_observer_.AlignOnce(phys_rotor_mech_revs, phys_elec_vel_rads, true);
				}

                // Lay ket qua luu vao status de report hoac feed nguoc lai FOC
                const auto& obs_state = sensorless_observer_.state();
                status_.sensorless_estimated_theta    = obs_state.estimated_theta;
                status_.sensorless_estimated_velocity = obs_state.estimated_velocity;
                status_.sensorless_confidence         = obs_state.confidence;
                status_.sensorless_emf_d              = obs_state.emf_d;
                status_.sensorless_emf_q              = obs_state.emf_q;
                status_.sensorless_time_active        = obs_state.time_active;
                // =================================================
  }
    ...
    case kBrake: {
      ...
      case kBrake: {
        if ((data->mode == kPosition || data->mode == kStayWithinBounds) &&
            !data->ignore_position_bounds && ISR_IsOutsideLimits()) {
          status_.mode = kFault;
          status_.fault = errc::kStartOutsideLimit;
        } else if ((data->mode == kPosition ||
                    data->mode == kStayWithinBounds) &&
                   !data->ignore_position_bounds && ISR_InvalidLimits()) {
          status_.mode = kFault;
        ...
        // ========================================================
		// DONG BO GOC SMO TU ENCODER KHI BAT DAU CHAY (SMO)
		// ========================================================
          // Dùng velocity_filt thay cho raw velocity để SMO không bị sốc lúc mới đề ba
                  const float phys_rotor_mech_revs = motor_position_config()->output.sign * (position_.position / motor_position_->config()->rotor_to_output_ratio);
                  const float phys_elec_vel_rads = motor_position_config()->output.sign * (position_.velocity / motor_position_->config()->rotor_to_output_ratio) * motor_.poles * 0.5f * k2Pi;
  ...
  bool ISR_IsOutsideLimits() {
    return ((!std::isnan(position_config_.position_min) &&
    		 status_.position < position_config_.position_min) || // Đổi thành status_.position
            (!std::isnan(position_config_.position_max) &&
             status_.position > position_config_.position_max));  // Đổi thành status_.position
  }
  ...
  void ISR_DoControl(const SinCos &sin_cos,
    ...
    float dt = rate_config_.period_s;
        ...
        if (status_.mode == kCurrent || status_.mode == kPosition || status_.mode == kStayWithinBounds || status_.mode == kVoltageDq) {
              float ud = old_d_V;
              float uq = old_q_V * motor_position_config()->output.sign;
              float id = status_.d_A;
              float iq = status_.q_A * motor_position_config()->output.sign;

              // SỬ DỤNG VẬN TỐC LAI: Khi alpha=1, RLS nhận vận tốc 100% từ ASMO
			float we_phys = motor_position_config()->output.sign * active_mech_hz_ * motor_.poles * 0.5f * k2Pi;
			parameter_estimator_.Update(id, iq, ud, uq, we_phys, dt, current_temp_C);

         } else if (status_.mode == kStopped) {
             parameter_estimator_.UpdateStandby(rate_config_.period_s, current_temp_C);
         }
    ...
    if ((status_.mode == kPosition || status_.mode == kStayWithinBounds) &&
        !std::isnan(status_.timeout_s) && status_.timeout_s <= 0.0f) {
  ...
  void ISR_DoBalancedVoltageControl(const Vec3 &voltage) MOTEUS_CCM_ATTRIBUTE {
    ...
    const Vec3 pwm_in = {voltage.a / bus_V, voltage.b / bus_V,
                         voltage.c / bus_V};
    ...
    ISR_DoPwmControl(
        Vec3{pwm_in.a - offset, pwm_in.b - offset, pwm_in.c - offset});
  }
  ...
  void ISR_DoVoltageFOC(CommandData *data) MOTEUS_CCM_ATTRIBUTE {
    ...
    const float max_voltage =
        (0.5f - rate_config_.min_pwm) * status_.filt_bus_V * kSvpwmRatio;
    InverseDqTransform idt(sc, Limit(data->voltage, -max_voltage, max_voltage),
                           0);
  ...
  void ISR_DoCurrent(const SinCos &sin_cos, float i_d_A_in, float i_q_A_in,
    ...
    // BYPASS: Nếu đang chạy 100% Sensorless thì bỏ qua lỗi mất góc từ Encoder (SMO)
        if (!position_.theta_valid && sensorless_alpha_blend_ < 1.0f) {
          status_.mode = kFault;
...
//    }
    ...
    auto limit_q_current = [&](std::pair<float, errc> in_pair)
        -> std::pair<float, errc>
            MOTEUS_CCM_ATTRIBUTE {
              const auto in = in_pair.first;
              if (ignore_position_bounds) {
                return {in, in_pair.second};
              }
              ...
              if (!std::isnan(position_config_.position_max) &&
                ...
                return std::make_pair(
                    in * std::max(0.0f, 1.0f - (status_.position - // Đổi thành status_.position
              ...
              if (!std::isnan(position_config_.position_min) &&
                  ...
                  in < 0.0f) {
                return std::make_pair(
                    in * std::max(0.0f, 1.0f - (position_config_.position_min -
                    							   status_.position) / // Đổi thành status_.position
                                                   config_.position_derate),
                    errc::kLimitPositionBounds);
              }
    ...
    auto limit_q_velocity = [&](std::pair<float, errc> in_pair)
        -> std::pair<float, errc>
            MOTEUS_CCM_ATTRIBUTE {
              const auto in = in_pair.first;
              ...
              const errc maybe_fault = derate_fraction < 1.0f
                                           ? errc::kLimitMaxVelocity
                                           : errc::kLimitMaxCurrent;
              return LimitCode(in, -current_limit, current_limit, maybe_fault,
                               in_pair.second);
            };

    float derate_fraction = (status_.filt_fet_temp_C - derate_temperature_) /
    ...
    const float derate_current_A =
        std::max<float>(0.0f, derate_fraction * (config_.derate_current_A -
                                                 config_.max_current_A) +
                                  config_.max_current_A);

    const float temp_limit_A =
        std::min<float>(config_.max_current_A, derate_current_A);

    auto limit_either_current =
        [&](std::pair<float, errc> in_pair) MOTEUS_CCM_ATTRIBUTE {
          return LimitCode(in_pair.first, -temp_limit_A, temp_limit_A,
                           derate_fault, in_pair.second);
        };

    const auto almost_i_q_A_pair = limit_either_current(limit_q_velocity(
        limit_q_current(std::make_pair(i_q_A_in, errc::kSuccess))));
    const auto almost_i_d_A_pair =
        limit_either_current(std::make_pair(i_d_A_in, errc::kSuccess));

    const auto almost_i_q_A = almost_i_q_A_pair.first;
    const auto almost_i_d_A = almost_i_d_A_pair.first;
    const auto almost_fault = (almost_i_q_A_pair.second == errc::kSuccess)
                                  ? almost_i_d_A_pair.second
                                  : almost_i_q_A_pair.second;
    ...
    const auto [i_d_A, i_q_A, limit_code] = [&]() {
      ...
      return std::make_tuple(scaled_d_power / (1.5f * old_d_V),
    ...
    const float max_V = rate_config_.max_voltage_ratio * kSvpwmRatio * 0.5f *
                        status_.filt_bus_V;

//    const float we = motor_position_config()->output.sign * position_.velocity / motor_position_->config()->rotor_to_output_ratio * motor_.poles * 0.5f * k2Pi;

    const auto limit_to_max_voltage = [max_V](float denorm_d_V,
                                              float denorm_q_V, errc inlimit) {
      ...
      return std::make_tuple(denorm_d_V * scale, denorm_q_V * scale,
                             errc::kLimitMaxVoltage);
    };


    // Dùng active_mech_hz_ cho bộ bù chéo (Decoupling) để không bơm nhiễu thẳng vào Vd, Vq làm sập SMO
        const float we_ctrl = active_mech_hz_ * motor_.poles * 0.5f * k2Pi;

        // TRẢ LẠI NGUYÊN BẢN V1: Trùng khớp tuyệt đối hệ số Tuning của Sensorless và Encoder
        const float bemf_voltage = feedforward_velocity_rotor * config_.bemf_feedforward * (rls_flux_hat_ * k2Pi);
    ...
    if (!config_.voltage_mode_control) {
    	const float denorm_d_V =
				  ...
				  we_ctrl * rls_Lq_hat_ * i_q_A;; // Sua status_.rls_Lq_hat thanh rls_Lq_hat_

    	      const float denorm_q_V =
    	          pid_q_.Apply(status_.q_A, i_q_A, rate_config_.rate_hz) +
    	          i_q_A * config_.current_feedforward * rls_R_hat_ +
				  we_ctrl * rls_Ld_hat_ * i_d_A + // Sua status_.rls_Ld_hat thanh rls_Ld_hat_
				  bemf_voltage;
      ...
      status_.pid_d.integral = Limit(status_.pid_d.integral, -max_V, max_V);

      status_.pid_q.integral = Limit(status_.pid_q.integral, -max_V, max_V);

      // eq 2.28 from "DYNAMIC MODEL OF PM SYNCHRONOUS MOTORS" D. Ohm,
      // 2000
      status_.power_W = 1.5f * (status_.d_A * d_V + status_.q_A * q_V);

      ISR_DoVoltageDQ(sin_cos, d_V, q_V);
    } else {
      status_.power_W = 1.5f * (i_d_A * i_d_A * motor_.resistance_ohm +
                                i_q_A * i_q_A * motor_.resistance_ohm);

      auto [d_V, q_V, final_limit_code] = limit_to_max_voltage(
          i_d_A * rls_R_hat_ - we_ctrl * rls_L_hat_ * i_q_A,
		  i_q_A * rls_R_hat_ + we_ctrl * rls_L_hat_ * i_d_A + bemf_voltage,
		  limit_code);
  ...
  // function.
  Vec3 ISR_CalculatePhaseVoltage(const SinCos &sin_cos, float d_V,
                                 float q_V) MOTEUS_CCM_ATTRIBUTE {
    ...
    InverseDqTransform idt(sin_cos, control_.d_V,
  ...
  void ISR_DoVoltageDQ(const SinCos &sin_cos, float d_V,
                       float q_V) MOTEUS_CCM_ATTRIBUTE {
    ISR_DoBalancedVoltageControl(ISR_CalculatePhaseVoltage(sin_cos, d_V, q_V));
  }

  void ISR_DoVoltageDQCommand(const SinCos &sin_cos, float d_V,
                              float q_V) MOTEUS_CCM_ATTRIBUTE {
    if (motor_.poles == 0) {
      // We aren't configured yet.
      status_.mode = kFault;
    ...
    ISR_DoBalancedVoltageControl(ISR_CalculatePhaseVoltage(
        sin_cos, Limit(d_V, -max_V, max_V), Limit(q_V, -max_V, max_V)));
  }
  ...
  void ISR_DoPositionTimeout(const SinCos &sin_cos,
    ...
    if (config_.timeout_mode == kStopped) {
      ...
      ISR_DoPositionCommon(sin_cos, &timeout_data, apply_options,
                           timeout_data.max_torque_Nm, 0.0f, 0.0f);
  ...
  void ISR_DoZeroVelocity(const SinCos& sin_cos, CommandData* data) MOTEUS_CCM_ATTRIBUTE {
    ...
    ISR_DoPositionCommon(sin_cos, &zero_velocity, apply_options,
                         config_.timeout_max_torque_Nm, 0.0f, 0.0f);
  }
  ...
  void ISR_DoPositionCommon(const SinCos &sin_cos, CommandData *data,
                            const PID::ApplyOptions &pid_options,
                            float max_torque_Nm, float feedforward_Nm,
                            float velocity) MOTEUS_CCM_ATTRIBUTE {
    const int64_t absolute_relative_delta =
        static_cast<int64_t>(motor_position_->absolute_relative_delta.load())
        << 32ll;

    const float velocity_command = BldcServoPosition::UpdateCommand(
        &status_, &config_, &position_config_, &position_,
        absolute_relative_delta, rate_config_.rate_hz, data, velocity);
    ...
    if (config_.fixed_voltage_mode ||
      ...
      status_.position = static_cast<float>(static_cast<int32_t>(
      ...
      if (!std::isnan(data->fixed_current_override)) {
        ISR_DoCurrent(sin_cos, data->fixed_current_override, 0.0f, 0.0f,
                      data->ignore_position_bounds);
      } else {
        const float fixed_voltage =
            std::isnan(data->fixed_voltage_override)
                ? config_.fixed_voltage_control_V +
                      (std::abs(status_.velocity) * v_per_hz_ *
...
//    }
    ...
    // BYPASS: Bỏ qua lỗi mất kết nối Encoder nếu đang bám dính SMO
        if (!position_.position_relative_valid && sensorless_alpha_blend_ < 1.0f) {
          status_.mode = kFault;
          status_.fault = errc::kPositionInvalid;
          return;
        }
        if (position_.error != MotorPosition::Status::kNone && sensorless_alpha_blend_ < 1.0f) {
          status_.mode = kFault;
          status_.fault = errc::kEncoderFault;
          return;
        }

    //Cắt nguồn dữ liệu Encoder của PID (SMO)
    // SỬA: Dùng status_.velocity (đã bị ghi đè bằng Hybrid) thay vì position_.velocity phần cứng
	const float measured_velocity =
			velocity_command + Threshold(status_.velocity - velocity_command,
    ...
    const float inertia_feedforward_Nm =
        2.0f * kPi * status_.control_acceleration * config_.inertia_feedforward;
    ...
    // SỬA: Dùng active_position_relative_raw_ thay vì position_.position_relative_raw
        const float unlimited_torque_Nm =
            (pid_position_.Apply(
                 (static_cast<int32_t>((active_position_relative_raw_ -
                                        *status_.control_position_raw) >>
                                       32) /
                  65536.0f),
                 0.0, measured_velocity, velocity_command, rate_config_.rate_hz,
                 pid_options) +
             feedforward_Nm + inertia_feedforward_Nm);
    ...
    const float limited_q_A = torque_to_current(
        limited_torque_Nm * motor_position_->config()->rotor_to_output_ratio);

    {
      const auto &pos_config = motor_position_->config();
      const auto commutation_source = pos_config->commutation_source;
      const float cpr =
          static_cast<float>(pos_config->sources[commutation_source].cpr);
      const float commutation_position =
          position_.sources[commutation_source].filtered_value / cpr;

      auto sample = [&](const auto &table, float scale) {
      ...
      const float q_comp_A =
          sample(motor_.cogging_dq_comp, motor_.cogging_dq_scale);
    ...
    const float q_A = is_torque_constant_configured()
                          ? compensated_q_A
                          : Limit(compensated_q_A, -kMaxUnconfiguredCurrent,
                                  kMaxUnconfiguredCurrent);

    const float d_A = [&]() MOTEUS_CCM_ATTRIBUTE {
      const auto error = (status_.filt_1ms_bus_V - flux_brake_min_voltage_);
  ...
  void ISR_DoStayWithinBounds(const SinCos &sin_cos,
    ...
    const auto target_position =
        [&]() MOTEUS_CCM_ATTRIBUTE -> std::optional<float> {
    ...
    data->position_relative_raw =
        MotorPosition::FloatToInt(*target_position) - absolute_relative_delta;
    data->velocity = 0.0;
  ...
  void ISR_DoMeasureInductance(const SinCos &sin_cos,
    ...
    const float d_V =
        Limit(offset + data->d_V * (status_.meas_ind_phase > 0 ? 1.0f : -1.0f),
              -max_V, max_V);

    // We also integrate the difference in current.
    status_.meas_ind_integrator +=
        (status_.d_A - status_.meas_ind_old_d_A) * old_sign_float;
    status_.meas_ind_old_d_A = status_.d_A;
  ...
  void ISR_MaybeEmitDebug() MOTEUS_CCM_ATTRIBUTE {
    if (config_.emit_debug == 0 || !debug_uart_) {
      return;
    }
...
//    ===== KHAI BAO BIEN SMO =====
   SensorlessObserver sensorless_observer_;
   float old_sin = 0.0f;
   float old_cos = 0.0f;
//    ==========

  const bool family0_rev4_and_older_ =
      (g_measured_hw_family == 0 && g_measured_hw_rev <= 4);
  const bool family0_ = (g_measured_hw_family == 0);
  const bool family1or2or3_ =
      (g_measured_hw_family == 1 || g_measured_hw_family == 2 ||
  ...
  // BIẾN ĐỂ LƯU TRẠNG THÁI HYBRID (SMO)
  float sensorless_alpha_blend_ = 0.0f;
  float active_mech_hz_ = 0.0f;
  int64_t active_position_relative_raw_ = 0; // Thêm biến này để nuôi PID Position
...
BldcServo::BldcServo(micro::Pool *pool,
                     ...
                     MotorDriver *motor_driver, AuxADC *aux_adc,
                     AuxPort *aux1_port, AuxPort *aux2_port,
                     MotorPosition *motor_position, const Options &options)
    : impl_(pool, persistent_config, telemetry_manager, millisecond_timer,
            motor_driver, aux_adc, aux1_port, aux2_port, motor_position,
            options) {}
BldcServo::~BldcServo() {}

void BldcServo::Start() { impl_->Start(); }

void BldcServo::PollMillisecond() { impl_->PollMillisecond(); }

void BldcServo::Command(const CommandData &data) { impl_->Command(data); }

const BldcServo::Status &BldcServo::status() const { return impl_->status(); }

const BldcServo::Config &BldcServo::config() const { return impl_->config(); }

const BldcServo::Control &BldcServo::control() const {
  return impl_->control();
}

const AuxPort::Status &BldcServo::aux1() const { return impl_->aux1(); }

const AuxPort::Status &BldcServo::aux2() const { return impl_->aux2(); }
...
void BldcServo::RequireReindex() { impl_->RequireReindex(); }

void BldcServo::RecapturePositionVelocity() {
  impl_->RecapturePositionVelocity();
}

void BldcServo::Fault(moteus::errc fault_code) { impl_->Fault(fault_code); }
