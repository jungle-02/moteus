...
#include "fw_smo2/error.h"
#include "fw_smo2/measured_hw_rev.h"
#include "fw_smo2/pid.h"
#include "fw_smo2/simple_pi.h"
...
struct BldcServoStatus {
  ...
  float cos = 0.0f;
	...
	// ===== SMO TELEMETRY FIELDS =====
	// Ước lượng vị trí từ sensorless observer
	float sensorless_estimated_theta = 0.0f;

	// Uoc luong toc do tu sensorless observer
	float sensorless_estimated_velocity = 0.0f;

	// Do tin cay cua observer (0.0-1.0)
	float sensorless_confidence = 0.0f;

	// Back EMF uoc luong truc D
	float sensorless_emf_d = 0.0f;

	// Back EMF uoc luong truc Q
	float sensorless_emf_q = 0.0f;

	// Time active (thoi gian hoat dong cua observer)
	float sensorless_time_active = 0.0f;
    // ==========
  ...
  template <typename Archive> void Serialize(Archive *a) {
    ...
    // ===== SERIALIZE SMO TELEMETRY =====
    a->Visit(MJ_NVP(sensorless_estimated_theta));
    a->Visit(MJ_NVP(sensorless_estimated_velocity));
    a->Visit(MJ_NVP(sensorless_confidence));
    a->Visit(MJ_NVP(sensorless_emf_d));
    a->Visit(MJ_NVP(sensorless_emf_q));
    a->Visit(MJ_NVP(sensorless_time_active));
    // ==========
...
struct BldcServoConfig {
  uint16_t pwm_rate_hz = (g_measured_hw_family == 0 && g_measured_hw_rev <= 2)
                             ? 60000
                         : g_measured_hw_family == 3 ? 15000
                                                     : 10000; // Tan so PWM mac dinh

  float current_sense_ohm = (g_measured_hw_family == 2 ? 0.002f : 0.0005f);

  // We pick a default maximum voltage based on the board revision.
  float max_voltage = g_measured_hw_family == 0
  ...
  float temperature_margin = (g_measured_hw_family == 3) ? 15.0f : 25.0f;
  float fault_temperature = (g_measured_hw_family == 3) ? 105.0f : 75.0f;
  ...
  float max_current_A = (g_measured_hw_family == 0 || g_measured_hw_family == 1)
                            ? 100.0f
                        : g_measured_hw_family == 2 ? 20.0f
                        : g_measured_hw_family == 3 ? 120.0f
                                                    : invalid_float();
  float derate_current_A =
      (g_measured_hw_family == 0 || g_measured_hw_family == 1 ||
...
template <> struct IsEnum<moteus::BldcServoMode> {
