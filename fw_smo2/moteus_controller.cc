...
#include "fw_smo2/moteus_controller.h"

#include "fw_smo2/aux_port.h"
#include "fw_smo2/drv8323.h"
#include "fw_smo2/moteus_hw.h"
#include "fw_smo2/motor_position.h"
...
enum class Register {
  ...
  // ===== SMO REGISTERS =====
  kSensorlessEstimatedTheta = 0x165,
  kSensorlessEstimatedVelocity = 0x166,
  // ==========
};
 ...
 public:
  ...
  WriteAction Write(multiplex::MicroServer::Register reg,
    ...
    switch (static_cast<Register>(reg)) {
      ...
      //Cam ghi de SMO
	  case Register::kSensorlessEstimatedTheta:
	  case Register::kSensorlessEstimatedVelocity:
  ...
  multiplex::MicroServer::ReadResult Read(
    ...
    switch (static_cast<Register>(reg)) {
	  ...
	  // ===== SMO REGISTERS =====
		case Register::kSensorlessEstimatedTheta: {
		  return ScalePosition(bldc_.status().sensorless_estimated_theta, type);
		}
		case Register::kSensorlessEstimatedVelocity: {
		  return ScaleVelocity(bldc_.status().sensorless_estimated_velocity, type);
		}
		// ==========
